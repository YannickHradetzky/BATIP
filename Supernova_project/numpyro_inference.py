...
class Config:
    ...
    OK_PRIOR = (-0.5, 0.5)   # Uniform prior range for Omega_k
...
class CosmologyInference:
    ...
    def chi(self, h0, Om, Ok, z, flat=True):
        ...
        N = 1000
        z_max = jnp.max(z)
        z_array = jnp.linspace(Config.MIN_REDSHIFT, z_max, N)
    ...
    def luminosity_distance(self, H0, Om, Ok, z, flat=True):
        ...
        chi_val = jnp.where(
            ...
            Config.C / sqrt_Ok * sin_term # else return closed universe
        )
        return (1 + z) * chi_val
    ...
    def model_curved(self, z, mu_obs, mu_err):
        ...
        H0 = sample("H0", dist.Uniform(*Config.H0_PRIOR))
        Om = sample("Om", dist.Uniform(*Config.OM_PRIOR))
        Ok = sample("Ok", dist.Uniform(*Config.OK_PRIOR))
...
if __name__ == "__main__":
    ...
    Ol = 1 - flat_samples['Om']
    flat_array = np.column_stack((
        np.array(flat_samples['H0']),
        np.array(flat_samples['Om']),
        np.array(Ol)
    ))

    # print the mean and std of the samples
    print(f"\nResults for flat model:")
    print("Parameter      Mean ± Std")
    print("-" * 30)
    print(f"H0:         {flat_array[:, 0].mean():.2f} ± {flat_array[:, 0].std():.2f}")
    print(f"Ωm:         {flat_array[:, 1].mean():.3f} ± {flat_array[:, 1].std():.3f}")
    print(f"ΩΛ:         {flat_array[:, 2].mean():.3f} ± {flat_array[:, 2].std():.3f}")
    print()
    
    # Run curved model
    print("\nRunning curved ΛCDM model...")
    curved_samples = cosmo.run_inference(model_type="curved")
    Ol = 1 - curved_samples['Om'] - curved_samples['Ok']
    curved_array = np.column_stack((
        np.array(curved_samples['H0']),
        np.array(curved_samples['Om']),
        np.array(curved_samples['Ok']),
        np.array(Ol)
    ))

    # print the mean and std of the samples
    print(f"\nResults for curved model:")
    print("Parameter      Mean ± Std")
    print("-" * 30)
    print(f"H0:         {curved_array[:, 0].mean():.2f} ± {curved_array[:, 0].std():.2f}")
    print(f"Ωm:         {curved_array[:, 1].mean():.3f} ± {curved_array[:, 1].std():.3f}")
    print(f"Ωk:         {curved_array[:, 2].mean():.3f} ± {curved_array[:, 2].std():.3f}")
    print(f"ΩΛ:         {curved_array[:, 3].mean():.3f} ± {curved_array[:, 3].std():.3f}")
    print()

    # Save the samples
    np.savetxt(Config.DATA_PATH + f'numpyro_samples_curved_{Config.NUM_WARMUP}_{Config.NUM_SAMPLES}.txt', curved_array)
    np.savetxt(Config.DATA_PATH + f'numpyro_samples_flat_{Config.NUM_WARMUP}_{Config.NUM_SAMPLES}.txt', flat_array)