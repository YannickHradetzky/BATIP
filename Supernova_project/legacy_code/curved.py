...
def hubble_z(H0, Om, Ok, z):
    ...
    lambda_term = (1 - Om - Ok)  # Since Ok = 0
...
def model(z: jnp.array, mu_obs: jnp.array, mu_err: jnp.array):
    ...
    Ok = sample("Ok", dist.Normal(0.0, 0.2))    # Centered around 0.0
...
Ok_mean = jnp.mean(samples['Ok'])
test_cosmological_calculations(H0_mean, Om_mean, Ok_mean, 'curved_model')
